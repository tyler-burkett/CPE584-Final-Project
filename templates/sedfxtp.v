...
`ifdef functional
  {? lib ?}_pg_U_MUX_2_1 ( mux_out , {? in[2] ?}${? in[1] ?} , {? in[3] ?} , {? in[4] ?} ) , ( {? in[2] ?}${? in[1] ?} , buf_Q , {? in[1] ?} , {? in[2] ?} ) ;
  `ifdef SC_USE_PG_PIN
...
`else
  ...
  specify
    ...
    $setuphold ( posedge {? in[0] ?} , posedge {? in[3] ?} , 0:0:0 , 0:0:0 , notifier , COND2 , COND2 , CLK_delayed , SCD_delayed ) ;
    $setuphold ( posedge {? in[0] ?} , negedge {? in[3] ?} , 0:0:0 , 0:0:0 , notifier , COND2 , COND2 , CLK_delayed , SCD_delayed ) ;
    $setuphold ( posedge {? in[0] ?} , posedge {? in[4] ?} , 0:0:0 , 0:0:0 , notifier , COND3 , COND3 , CLK_delayed , SCE_delayed ) ;
    $setuphold ( posedge {? in[0] ?} , negedge {? in[4] ?} , 0:0:0 , 0:0:0 , notifier , COND3 , COND3 , CLK_delayed , SCE_delayed ) ;
