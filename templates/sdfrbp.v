{#
	DESCRIPTION
		Scan D-Flip Flop with async reset active low (q and qb)
	PARAMETER MAPPING
		in[0]	:	CLK
		in[1]	:	D
		in[2]	:	SCD
...
`ifdef SC_USE_PG_PIN
 {? lib ?}_pg_U_DF_P_R_NO_pg #0.001 ( buf_Q , mux_out , {? in[0] ?} , reset ,  , {? vpwr[0] ?} , {? vgnd[0] ?} ) ;
`else
 {? lib ?}_pg_U_DF_P_R #0.001 ( buf_Q , mux_out , {? in[0] ?} , reset ) ;
`endif 
`else
  reg notifier ; 
  wire D_delayed;
...
{? lib ?}_pg_U_DF_P_R_NO_pg ( buf_Q , mux_out , CLK_delayed , reset , notifier , {? vpwr[0] ?} , {? vgnd[0] ?} ) ; 
  ...
  specify
    ...
    $recrem ( posedge {? in[4] ?} , posedge {? in[0] ?} , 0:0:0 , 0:0:0 , notifier , AWAKE , AWAKE , RESETB_delayed , CLK_delayed ) ; 
