...
`ifdef functional
`ifdef SC_USE_PG_PIN
 {?lib?}_pg_U_DL_P_NO_pg #0.001 ( buf_Q , {?D0?} , {?GATE0?} ,  , {?vpwr0?} , {?vgnd0?} ) ;
`else
 {?lib?}_pg_U_DL_P #0.001 ( buf_Q , {?D0?} , {?GATE0?} ) ;
`endif 
`else
  wire GATE_delayed;
  wire D_delayed;
...
`else
  ...
  assign AWAKE = ( {?vpwr0?} === 1'b1 ) ; 
    ...
    ( {?D0?} -=> {?out1?} ) = ( 0:0:0 , 0:0:0 ) ;  // delays are tris , tfall
		 ...
		 ( posedge {?GATE0?} => ( {?out1?} -: {?D0?} ) ) = ( 0:0:0 , 0:0:0 ) ; // delays are  tris , tfall
