...
//             ASYNCHRONOUS CLEAR ( {? out[0] ?} OUTPUT UDP ).
    ...
    //  {? in[0] ?}  {? in[1] ?}   {? in[2] ?}    {? in[3] ?}  {? in[4] ?} {? in[5] ?} : Qt : Qt+1
        *  b    0     ?     1    0  : ?  :  -  ; // data event, hold unless {? in[1] ?}==x
        ...
        0  r    ?     ?     1    0  : ?  :  0  ; // clock data on {? in[1] ?}
        1  r    0     ?     1    0  : ?  :  1  ; // clock data on {? in[1] ?}

        0  (x1) ?     ?     1    0  : 0  :  0  ; // possible {? in[1] ?}, hold when {? in[0] ?}=={? out[0] ?}==0
        1  (x1) 0     ?     1    0  : 1  :  1  ; // possible {? in[1] ?}, hold when {? in[0] ?}=={? out[0] ?}==1

        0  x    ?     ?     1    0  : 0  :  0  ; // unkown {? in[1] ?}, hold when {? in[0] ?}=={? out[0] ?}==0
        1  x    0     ?     1    0  : 1  :  1  ; // unkown {? in[1] ?}, hold when {? in[0] ?}=={? out[0] ?}==1

        ?  b    (?x)  ?     1    0  : 0  :  0  ; // {? in[2] ?}=>x, hold when {? out[0] ?}==0 unless {? in[1] ?}==x
